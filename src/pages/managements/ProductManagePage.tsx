    ...
    },
    {
      ...
      render: (sellerName: string) => {
        if (sellerName === 'DELETED SELLER')
          return <Tag color='#F08080' style={{ color: '#7D6B91' }}>{sellerName}</Tag>;
        return <Tag color='#C3AED6' style={{ color: '#7D6B91' }}>{sellerName}</Tag>;
      },
    },
    {
      title: 'Action',
...
const SellProductModal = ({ product }: { product: IProduct & { key: string } }) => {
  ...
  return (
    <>
      ...
      <Modal
        title={<span style={{ color: '#7D6B91' }}>Sell Product</span>}
        open={isModalOpen}
